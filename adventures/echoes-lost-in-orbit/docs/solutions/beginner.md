...
```bash
adventures/echoes-lost-in-orbit/beginner/manifests/appset.yaml
```
...
spec:
  generators:
    - git:
        ...
        revision: HEAD
        directories:
          - path: adventures/echoes-lost-in-orbit/beginner/manifests/overlays/*
  template:
    metadata:
      name: echo-server
      labels:
    ...
    spec:
      ...
      source:
        repoURL: __REPO_URL__
        targetRevision: HEAD
        path: adventures/echoes-lost-in-orbit/beginner/manifests/overlays/{{path.basename}}
      destination:
...
```bash
kubectl apply -n argocd -f adventures/echoes-lost-in-orbit/beginner/manifests/appset.yaml
```
