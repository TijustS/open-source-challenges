...
Or, we can simply look at the file in the repo at `adventures/echoes-lost-in-orbit/intermediate/manifests/base/analysis-template.yaml`.
...
This outputs one service called `prometheus-server`, not `prom-server`. It looks like there's a typo in our manifest. Let's fix it by changing `prom-server` to `prometheus-server` in `adventures/echoes-lost-in-orbit/intermediate/manifests/base/analysis-template.yaml`.
